...
function protocoloChamados(nomeCliente) {
  return [
    ...
    'acumulado sozinho.',
    '',
    '## Anexo binario: extraia uma vez, leia o .md pra sempre',
    '',
    'Workbook, spec, manual — `.docx`, `.pdf`, `.pptx`, `.xlsx` nao se leem direto:',
    'cada leitura custa script e round-trip, e a proxima sessao paga tudo de novo.',
    '',
    '1. **Procure o `.md` irmao antes de abrir o binario.** Existe? Leia so ele. O',
    '   original so volta se o usuario disser que o arquivo mudou.',
    '2. Nao existe? **Extraia uma vez** e grave o markdown ao lado do que ele serve:',
    '   vale pro cliente inteiro (workbook, padrao de desenvolvimento, manual de',
    '   processo) → `docs/<nome>.md`, citado no indice do `docs/README.md`; e de uma',
    '   frente so (spec, ata) → `chamados/<ID>/<nome>.md`.',
    '3. Primeira linha do arquivo gerado: `> Extraido de <caminho do original> em',
    '   <dd.mm.aaaa>.` — quem conferir precisa saber de onde veio e se envelheceu.',
    '',
    'Converta, nao resuma: tabela vira tabela, secao vira secao. Resumo apaga',
    'justamente o detalhe (prefixo, pacote, excecao) que faz o documento valer.',
    'Capitulo que claramente nao interessa pode ficar de fora — diga numa linha o',
    'que voce cortou.',
    '',
    ...
    '   mandou o arquivo, extraia como manda a secao do anexo binario.',
