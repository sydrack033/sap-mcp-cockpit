...
module.exports = {
  ...
  // O que a UI liga/desliga por engine. O modal LE isto: o seletor de modo e
  // montado a partir de `modes`, entao modo que o binario aceita e nao esta
  // aqui simplesmente deixa de existir pro usuario.
  caps: {
    ...
    modes: ['focused', 'expert', 'hyperfocused']
  },
  ...
  // O que vai pro .env da PASTA. No vsp e igual ao env do processo porque o nome
  // da variavel carrega o profile (VSP_<ID>_PASSWORD), entao duas conexoes na
  // mesma pasta nao se atropelam. Nem todo engine tem essa sorte -- ver arc1.js.
  dotenvVars(id) { return this.passwordVars(id); },
  ...
  // opts.protocolo: quando o protocolo de trabalho esta ligado, a pasta tem docs/ e
  // chamados/ e quem manda no escopo de leitura e o proprio protocolo. Desligado, o
  // workspace e so config de acesso e vale avisar que nao ha o que vasculhar.
  instructions(envs, opts) {
    const L = [];
    L.push('# Workspace SAP MCP Cockpit — ambientes SAP via vsp (MCP)');
    L.push('');
    L.push('Workspace de **um cliente**: acesso aos sistemas SAP dele pelo `vsp`');
    L.push('(vibing-steampunk) como servidores **MCP**.');
